...
export class TrivyWrapper {
  ...
  updateTrivyVersion() {
    if (!this.checkTrivyInstalled()) {
      vscode.window.showErrorMessage(
        "Trivy not found. Please install Trivy first."
      );
      return;
    }
    ...
    try {
      // Run trivy update command
      const binary = this.getBinaryPath();
      const command = [binary, "update"];

      this.outputChannel.show();
      this.outputChannel.appendLine("Updating Trivy...");

      const result = child.execSync(command.join(" "));
      this.outputChannel.appendLine(result.toString());

      const newVersion = this.getInstalledTrivyVersion();
      vscode.window.showInformationMessage(
        `Trivy updated successfully to version ${newVersion}`
      );
    } catch (error: any) {
      this.outputChannel.appendLine(`Failed to update Trivy: ${error.message}`);
      vscode.window.showErrorMessage(
        "Failed to update Trivy. Check output for details."
      );
    }
  }
}
